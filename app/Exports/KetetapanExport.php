...
class KetetapanExport 
{
   ...
   protected $search;
    ...
    {
        // Path ke template
        $templatePath = storage_path('app/templates/temp-export-dokumen-ketetapan.xlsx');
        ...
        // 🔧 Query Ketetapan
        $query = Ketetapan::select('id', 'title', 'description', 'year_published', 'status')
        ...
        $ketetapans = $query->orderBy('id', 'asc')->get();

        // dd($ketetapans);

        // Hitung total
        $totalKetetapan = $ketetapans->count();
        $now = Carbon::now();

        // Tulis ke spreadsheet
        $sheet->setCellValue('D25', $totalKetetapan); // Total Ketetapan
        $sheet->setCellValue('D26', $now->format('d-m-Y H:i:s')); // Tanggal Export

        // Mulai dari baris 8
        $startRow = 8;
        foreach ($ketetapans as $index => $ketetapan) {
            $row = $startRow + $index;
            $sheet->setCellValueExplicit("B{$row}", $index + 1, DataType::TYPE_STRING); // No
            $sheet->setCellValue("C{$row}", $ketetapan->title); // Title
            $sheet->setCellValue("D{$row}", $ketetapan->description); // Description
            $sheet->setCellValue("E{$row}", $ketetapan->year_published); // Year Published
            $sheet->setCellValue("F{$row}", ucfirst($ketetapan->status)); // Status
        }
