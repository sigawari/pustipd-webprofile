...
export class Structure {
    ...
    previewCarousel() {
        ...
        // Create carousel items - Kepala pertama, kemudian member
        const allMembers = [];
        ...
        // Add member from all divisions (urutan sesuai input)
        formData.divisions.forEach((division) => {
            division.member.forEach((member) => {
                allMembers.push({
                    name: member.name,
    ...
    // Preview OrgChart - ONLY renders to modal content, NOT to main page
    previewOrgChart() {
        const formData = this.collectFormData();
        ...
        // Transform data untuk kompatibilitas dengan renderer
        const transformedData = {
            name: formData.name,
            description: formData.description,
            head: formData.head.name
                ? {
                      nama: formData.head.name,
                      jabatan: formData.head.position,
                      email: formData.head.email,
                      image: formData.head.photo,
                  }
                : null,
            divisions: formData.divisions.map((division) => ({
                name: division.name,
                members: division.member.map((member) => ({
                    nama: member.name,
                    jabatan: member.position,
                    email: member.email,
                    image: member.photo || "/assets/img/placeholder/dummy.png",
                })),
            })),
        };
        ...
        if (
            ...
            transformedData.divisions.length === 0
        ) {
            container.innerHTML = `
            ...
            this.renderStructurePreview(container, transformedData);
        }
    ...
    renderStructurePreview(container, formData) {
        ...
        formData.divisions.forEach((division, index) => {
            if (!division.member || division.member.length === 0) return;
