<x-public.layouts title="{{ $title }}" description="{{ $description }}" keywords="{{ $keywords }}">
    ...
    <section id="public-info" class="py-20 mt-8 bg-primary">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-col space-y-4 lg:space-y-0 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-3">
                            ...
                            <div class="text-sm text-gray-600 font-medium">
                                <span id="info-count">{{ $sops->total() }} dokumen tersedia</span>
                            </div>
            ...
            <div class="max-w-6xl mx-auto bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden">
                    ...
                    <table class="w-full min-w-[640px]">
                        ...
                        <thead>
                            <tr class="bg-secondary border-b border-blue-600">
                                ...
                                <th
                                    class="hidden md:table-cell px-6 py-4 text-left text-sm font-semibold text-white uppercase tracking-wider">
                                    Deskripsi
                                </th>
                                <th
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        @if ($sop->file_path && file_exists(storage_path('app/public/' . $sop->file_path)))
                                            <input type="checkbox"
                                                class="file-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
                                    ...
                                    <!-- Description (disembunyikan di mobile) -->
                                    <td class="hidden md:table-cell px-6 py-4 text-sm text-secondary leading-relaxed">
                                        <div class="max-w-sm">
                                            {{ Str::limit(strip_tags($sop->description), 120) }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($sop->file_path && file_exists(storage_path('app/public/' . $sop->file_path)))
                                            <a href="{{ route('public.document.download', ['tipe' => 'sop', 'id' => $sop->id]) }}"
                                                class="inline-flex items-center px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-xs font-medium shadow-sm"
    ...
    <!-- Bulk Download Form -->
    <form id="bulk-download-form" method="POST" action="{{ route('public.sop.bulk-download') }}"
        style="display: none;">
        @csrf
