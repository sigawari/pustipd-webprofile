<x-admin.layouts>
    ...
    @section('page-title', 'Pencapaian PUSTIPD')
    @section('page-description', 'Kelola konten Pencapaian PUSTIPD yang akan masuk ke beranda web PUSTIPD')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 m-3 sm:m-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <!-- Button Export -->
                <a href="{{ route('admin.beranda.pencapaian.export', [
        ...
        <script>
            window.bulkActionRoute = "{{ route('admin.beranda.pencapaian.bulk') }}";
        </script>
        ...
        <div class="flex flex-col gap-3 mb-4 sm:mb-6">
            <div class="relative">
                ...
                <input type="search" id="search-input" value="{{ request('search') }}"
                    data-url="{{ route('admin.beranda.pencapaian.index') }}" data-target="pencapaianTableBody"
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                <select id="filter-select" name="filter" data-url="{{ route('admin.beranda.pencapaian.index') }}"
                    data-target="pencapaianTableBody"
                ...
                <select id="perpage-select" name="perPage" data-url="{{ route('admin.beranda.pencapaian.index') }}"
                    data-target="pencapaianTableBody"
            ...
            <div class="min-w-full inline-block align-middle">
                <div class="overflow-hidden border border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="pencapaianTableBody" class="bg-white divide-y divide-gray-200">
                            @include('admin.Beranda.Pencapaian.partials.table_body')
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4 pt-4">
            ...
            <div class="text-sm text-gray-500 text-center sm:text-left">
                Menampilkan {{ $pencapaians->firstItem() }} sampai {{ $pencapaians->lastItem() }} dari
                {{ $pencapaians->total() }}
                {{ strtolower($title) }}
            </div>
            ...
            <div class="flex justify-center sm:justify-end">
                <nav class="inline-flex space-x-1 sm:space-x-2" aria-label="Pagination">
                    ...
                    @else
                        <a href="{{ $pencapaians->previousPageUrl() }}"
                    ...
                    @foreach ($pencapaians->getUrlRange(1, $pencapaians->lastPage()) as $page => $url)
                        @if ($page == $pencapaians->currentPage())
                            {{-- Halaman aktif --}}
                            <span
                    ...
                    <!-- Tombol Selanjutnya -->
                    @if ($pencapaians->hasMorePages())
                        <a href="{{ $pencapaians->nextPageUrl() }}"
                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center gap-1">
    ...
    @include('admin.beranda.pencapaian.create')
    @include('admin.beranda.pencapaian.update')
    @include('admin.beranda.pencapaian.delete')
</x-admin.layouts>
