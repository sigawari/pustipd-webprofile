        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-2">
                ...
                <button onclick="toggleStatus(this)"
                    ...
                    title="{{ $mitra->status === 'draft' ? 'Publish' : 'Unpublish' }}" data-id="{{ $mitra->id }}"
                    data-status="{{ $mitra->status }}"
                    data-url="{{ route('admin.beranda.mitra.toggle-visibility', $mitra->id) }}">

                    {{-- Eye (untuk Draft → Publish) --}}
                    <svg class="size-5 icon-eye text-green-500 {{ $mitra->status === 'draft' ? '' : 'hidden' }}"
                    ...
                    <svg class="size-5 icon-eye-off text-neutral-500 {{ $mitra->status === 'published' ? '' : 'hidden' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.5">
