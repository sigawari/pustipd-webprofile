...
@foreach ($mitras as $mitra)
    <div id="UpdateModal-{{ $mitra->id }}"
        ...
        <div
            ...
            <form method="POST" action="{{ route('admin.beranda.mitra.update', $mitra->id) }}">
                @csrf
                @method('PUT')

                <!-- Pertanyaan -->
                <div class="mb-4">
                    <label for="name-{{ $mitra->id }}"
                        class="block text-sm font-medium text-gray-700 mb-2">Pertanyaan</label>
                    <textarea name="name" id="name-{{ $mitra->id }}" rows="2" required
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ $mitra->name }}</textarea>
                </div>

                <!-- Logo -->
                <div class="mb-4">
                    <label for="image-{{ $mitra->id }}"
                        class="block text-sm font-medium text-gray-700 mb-2">Jawaban</label>
                    <textarea name="image" id="image-{{ $mitra->id }}" rows="4" required
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ $mitra->image }}</textarea>
                </div>
