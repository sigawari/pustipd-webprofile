        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-2">
                ...
                <button onclick="toggleStatus(this)"
                    ...
                    title="{{ $layanan->status === 'draft' ? 'Publish' : 'Unpublish' }}" data-id="{{ $layanan->id }}"
                    data-status="{{ $layanan->status }}"
                    data-url="{{ route('admin.beranda.layanan.toggle-visibility', $layanan->id) }}">

                    {{-- Eye (untuk Draft → Publish) --}}
                    <svg class="size-5 icon-eye text-green-500 {{ $layanan->status === 'draft' ? '' : 'hidden' }}"
                    ...
                    <svg class="size-5 icon-eye-off text-neutral-500 {{ $layanan->status === 'published' ? '' : 'hidden' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.5">
