...
@foreach ($layanans as $layanan)
    <div id="DeleteModal-{{ $layanan->id }}"
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
            ...
            <p class="text-gray-600 mb-4">Apakah Anda yakin ingin menghapus {{ strtolower($title) }} berikut?</p>

            <blockquote class="border-l-4 border-blue-500 pl-4 italic mb-6 text-gray-700 bg-gray-50 p-3 rounded">
                "{{ $layanan->name }}"
            </blockquote>

            <form method="POST" action="{{ route('admin.beranda.layanan.destroy', $layanan->id) }}">
                @csrf
                @method('DELETE')

                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeDeleteModal('{{ $layanan->id }}')"
