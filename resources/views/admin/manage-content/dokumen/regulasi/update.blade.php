@foreach ($regulasis as $regulasi)
    <!-- Modal Edit Panduan -->
    <div id="UpdateModal-{{ $regulasi->id }}"
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <form id="editForm" method="POST"
                action="{{ route('admin.manage-content.dokumen.regulasi.update', $regulasi->id) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="edit_title" required
                        value="{{ old('title', $regulasi->title) }}" placeholder="Masukkan judul regulasi.."
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="edit_description" rows="4" required placeholder="Tulis deskripsi regulasi.."
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg 
                            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $regulasi->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="year_published" id="edit_year_published" min="1900"
                        max="{{ date('Y') + 1 }}" value="{{ old('year_published', $regulasi->year_published) }}"
                        placeholder="{{ date('Y') }}"
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg 
                            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <input type="hidden" name="status" value="{{ old('status', $regulasi->status) }}">

                <!-- Tombol -->
                <div class="flex justify-end space-x-2 mt-6">
                    <button type="button" onclick="closeUpdateModal('{{ $regulasi->id }}')"
            ...
            <!-- Tombol X di pojok -->
            <button onclick="closeUpdateModal('{{ $regulasi->id }}')"
