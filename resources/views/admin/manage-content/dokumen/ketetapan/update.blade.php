@foreach ($ketetapans as $ketetapan )
    ...
    <div id="UpdateModal-{{ $ketetapan->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <!-- Form -->
            <form id="editForm" method="POST" action="{{ route('admin.manage-content.dokumen.ketetapan.update', $ketetapan->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="edit_title" required
                        value="{{ old('title', $ketetapan->title) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="edit_description" rows="4" required
                        placeholder="Tulis deskripsi ketetapan..."
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg 
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="year_published" id="edit_year_published" min="1900" max="{{ date('Y') + 1 }}"
                        value="{{ old('year_published', $ketetapan->year_published) }}"
                        placeholder="{{ date('Y') }}"
                        class="w-full px-3 py-2 border border-gray-200 rounded-lg 
                            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <!-- Tombol X di pojok -->
            <button onclick="closeUpdateModal('{{ $ketetapan->id }}')" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600">
