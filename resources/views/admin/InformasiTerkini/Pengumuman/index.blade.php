<x-admin.layouts>
    ...
    @section('page-title', 'Pengumuman PUSTIPD')
    @section('page-description', content: 'Kelola konten pengumuman')
    @section('breadcrumb')
        <li>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 m-3 sm:m-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1 text-sm">Kelola pengumuman berdasarkan kategori yang akan ditampilkan di
                    halaman pengumuman</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <!-- Button Export -->
                <a href="{{ route('admin.informasi-terkini.kelola-pengumuman.export', [
        ...
        <script>
            window.bulkActionRoute = "{{ route('admin.informasi-terkini.kelola-pengumuman.bulk') }}";
        </script>
        ...
        <div class="flex flex-col gap-3 mb-4 sm:mb-6">
            <div class="relative">
                ...
                <input type="search" id="searchInput" placeholder="Cari {{ $title }}..."
                    data-url="{{ route('admin.informasi-terkini.kelola-pengumuman.index') }}"
                    data-target="pengumumanTableBody"
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                <select id="categoryFilter" data-url="{{ route('admin.informasi-terkini.kelola-pengumuman.index') }}"
                    data-target="pengumumanTableBody"
                ...
                <select id="statusFilter" data-url="{{ route('admin.informasi-terkini.kelola-pengumuman.index') }}"
                    data-target="pengumumanTableBody"
                    class="flex-1 px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            ...
            <div class="min-w-full inline-block align-middle">
                <div class="overflow-hidden border border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200" style="min-width: 900px;">
                        ...
                        <tbody id="pengumumanTableBody" class="bg-white divide-y divide-gray-200">
                            @include('admin.InformasiTerkini.Pengumuman.partials.table_body')
                        </tbody>
                    </table>
                </div>
            </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4 pt-4">
            ...
            <div class="text-sm text-gray-500 text-center sm:text-left">
                Menampilkan {{ $kelolaPengumumans->firstItem() }} sampai {{ $kelolaPengumumans->lastItem() }} dari
                {{ $kelolaPengumumans->total() }}
                {{ strtolower($title) }}
            </div>
            ...
            <div class="flex justify-center sm:justify-end">
                <nav class="inline-flex space-x-1 sm:space-x-2" aria-label="Pagination">
                    <!-- Tombol Sebelumnya -->
                    @if ($kelolaPengumumans->onFirstPage())
                        <span
                    ...
                    @else
                        <a href="{{ $kelolaPengumumans->previousPageUrl() }}"
                    ...
                    @foreach ($kelolaPengumumans->getUrlRange(1, $kelolaPengumumans->lastPage()) as $page => $url)
                        @if ($page == $kelolaPengumumans->currentPage())
                            {{-- Halaman aktif --}}
                            <span
                    ...
                    <!-- Tombol Selanjutnya -->
                    @if ($kelolaPengumumans->hasMorePages())
                        <a href="{{ $kelolaPengumumans->nextPageUrl() }}"
                            class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center gap-1">
        ...
        @include('admin.InformasiTerkini.Pengumuman.create')
        @include('admin.InformasiTerkini.Pengumuman.update')
        @include('admin.InformasiTerkini.Pengumuman.delete')
