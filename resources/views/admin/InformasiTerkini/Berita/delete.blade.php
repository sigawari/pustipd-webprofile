...
@foreach ($kelolaBeritas as $berita)
    <div id="DeleteModal-{{ $berita->id }}"
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
            ...
            <form method="POST" action="{{ route('admin.informasi-terkini.kelola-berita.destroy', $berita->id) }}">
                @csrf
                @method('DELETE')

                <div class="flex justify-end space-x-2">
                    <button type="button" onclick="closeDeleteModal('{{ $berita->id }}')"
            ...
            <!-- Tombol X di pojok -->
            <button onclick="closeDeleteModal('{{ $berita->id }}')"
