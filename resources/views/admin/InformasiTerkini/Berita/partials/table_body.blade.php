        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-2">
                ...
                <button onclick="toggleStatus(this)"
                    ...
                    title="{{ $berita->status === 'draft' ? 'Publish' : 'Unpublish' }}" data-id="{{ $berita->id }}"
                    data-status="{{ $berita->status }}"
                    data-url="{{ route('admin.informasi-terkini.kelola-berita.toggle-visibility', $berita->id) }}">

                    {{-- Eye (untuk Draft → Publish) --}}
                    <svg class="size-5 icon-eye text-green-500 {{ $berita->status === 'draft' ? '' : 'hidden' }}"
                    ...
                    <svg class="size-5 icon-eye-off text-neutral-500 {{ $berita->status === 'published' ? '' : 'hidden' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.5">
...
@empty
    <tr class="hover:bg-gray-50">
        <td colspan="10" class="px-6 py-4 text-center text-gray-500 italic min-w-screen">
