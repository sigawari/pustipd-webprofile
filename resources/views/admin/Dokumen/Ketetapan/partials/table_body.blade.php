        ...
        <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-1">
                ...
                <button onclick="toggleStatus(this)"
                    ...
                    title="{{ $ketetapan->status === 'draft' ? 'Publish' : 'Unpublish' }}"
                    data-id="{{ $ketetapan->id }}" data-status="{{ $ketetapan->status }}"
                    data-url="{{ route('admin.dokumen.ketetapan.toggle-visibility', $ketetapan->id) }}">

                    {{-- Eye (untuk Draft → Publish) --}}
                    <svg class="size-5 icon-eye text-green-500 {{ $ketetapan->status === 'draft' ? '' : 'hidden' }}"
                    ...
                    <svg class="size-5 icon-eye-off text-neutral-500 {{ $ketetapan->status === 'published' ? '' : 'hidden' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.5">
                ...
                {{-- Download Button --}}
                @if ($ketetapan->file_path && $ketetapan->file_exists)
                    <a href="{{ route('admin.dokumen.ketetapan.download', $ketetapan->id) }}"
