...
@forelse ($panduans as $key => $panduan)
    <tr class="hover:bg-gray-50">
        <!-- Checkbox -->
        <td class="px-4 py-4 text-center">
            <input type="checkbox" class="item-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                value="{{ $panduan->id }}" onchange="updateBulkActionsBar()">
        </td>

        <!-- No. -->
        <td class="px-3 py-4 text-center whitespace-nowrap">
            {{ $panduans->firstItem() + $key }}
        </td>

        <!-- Nama Panduan -->
        <td class="px-6 py-4">
            <div class="text-sm font-medium text-gray-900 max-w-xs">
                {{ Str::limit($panduan->title, 50) }}
                <br>
                @if ($panduan->original_filename)
                    <span class="text-xs text-gray-600">📄 {{ $panduan->original_filename }}</span>
                @else
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            @if ($panduan->file_path && $panduan->file_exists)
                <div class="flex items-center space-x-1">
                    ...
                    <div class="text-xs">
                        <div class="text-green-600 font-medium">{{ strtoupper($panduan->file_type ?? 'FILE') }}</div>
                        @if ($panduan->formatted_file_size)
                            <div class="text-gray-500">{{ $panduan->formatted_file_size }}</div>
                        @endif
                    </div>
                </div>
            @elseif($panduan->file_path && !$panduan->file_exists)
                <div class="flex items-center space-x-1">
        ...
        <td class="px-3 py-4 text-center whitespace-nowrap">
            <span @class([
                ...
                'bg-green-300 text-green-800' => $panduan->status === 'published',
                'bg-yellow-300 text-yellow-800' => $panduan->status === 'draft',
                'bg-gray-300 text-gray-800' => $panduan->status === 'archived',
            ])>
                {{ ucfirst($panduan->status) }}
            </span>
        </td>
        ...
        <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-1">
                ...
                @if ($panduan->status === 'published')
                    <button onclick="quickStatusChange('{{ $panduan->id }}', 'draft')"
                ...
                @else
                    <button onclick="quickStatusChange('{{ $panduan->id }}', 'published')"
                ...
                <button onclick="openUpdateModal('{{ $panduan->id }}')"
                ...
                <button onclick="toggleVisibility(this)" data-id="{{ $panduan->id }}"
                    data-status="{{ $panduan->status }}"
                    class="p-1 rounded {{ $panduan->status === 'published' ? 'text-green-600 hover:text-green-900 hover:bg-green-50' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50' }}"
                    title="{{ $panduan->status === 'published' ? 'Sembunyikan dari publik' : 'Tampilkan di publik' }}">
                ...
                {{-- Download Button --}}
                @if ($panduan->file_path && $panduan->file_exists)
                    <a href="{{ route('admin.manage-content.dokumen.panduan.download', $panduan->id) }}"
                        class="p-1 text-purple-600 rounded hover:text-purple-900 hover:bg-purple-50"
                        title="Download {{ $panduan->original_filename }}" target="_blank">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                {{-- Delete Button --}}
                <button onclick="openDeleteModal('{{ $panduan->id }}')"
    ...
    <tr class="hover:bg-gray-50">
        <td colspan="8" class="px-6 py-4 text-center text-gray-500 italic">
            <div class="flex flex-col items-center justify-center text-sm text-gray-500 space-y-1">
                @if ($panduans->isEmpty() && !request()->filled('search') && !request()->filled('filter'))
                    ...
                    <p class="text-gray-400">Klik "Tambah {{ $title }}" untuk membuat {{ $title }}
                        pertama</p>
                @elseif ($panduans->isEmpty() && request()->filled('search'))
                ...
                @elseif ($panduans->isEmpty() && request()->filled('filter'))
