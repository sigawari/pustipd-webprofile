        ...
        <td class="px-3 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex justify-center space-x-1">
                ...
                <button onclick="toggleStatus(this)"
                    ...
                    title="{{ $panduan->status === 'draft' ? 'Publish' : 'Unpublish' }}" data-id="{{ $panduan->id }}"
                    data-status="{{ $panduan->status }}"
                    data-url="{{ route('admin.dokumen.panduan.toggle-visibility', $panduan->id) }}">

                    {{-- Eye (untuk Draft → Publish) --}}
                    <svg class="size-5 icon-eye text-green-500 {{ $panduan->status === 'draft' ? '' : 'hidden' }}"
                    ...
                    <svg class="size-5 icon-eye-off text-neutral-500 {{ $panduan->status === 'published' ? '' : 'hidden' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="1.5">
