            ...
            <div class="flex flex-wrap gap-2 justify-center">
                ...
                @if ($gallery->status !== 'published')
                    <button onclick="toggleVisibility(this)"
                        class="text-green-700 hover:text-white p-1 rounded hover:bg-green-700" title="Publis Gambar"
                        data-id="{{ $gallery->id }}" data-visibility="{{ $gallery->is_hidden ? 'true' : 'false' }}"
                ...
                @if ($gallery->status == 'published')
                    <button onclick="toggleVisibility(this)"
                        ...
                        title="Tampilkan/Sembunyikan" data-id="{{ $gallery->id }}"
                        data-visibility="{{ $gallery->is_hidden ? 'true' : 'false' }}"
                        data-url="{{ route('admin.tentang-kami.gallery.toggle-visibility', $gallery->id) }}">
                        <!--  -->
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-4 icon-hidden hidden">
                            <path stroke-linecap="round" stroke-linejoin="round"
