{{-- Modal Delete --}}
@if (isset($profilData))
    <div id="DeleteModal-{{ $profilData->id }}" class="hidden fixed inset-0 z-50 bg-black/50 items-center justify-center px-4">
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-md w-full text-center relative">
            <div class="flex flex-col items-center space-y-4">
                ...
                <h2 class="text-xl font-semibold text-gray-800">Yakin ingin menghapus semua data profil?</h2>
                <p class="text-gray-500 text-sm">Tindakan ini tidak dapat dibatalkan.</p>

                <!-- Form Delete -->
                <form method="POST" action="{{ route('admin.tentang-kami.profil.destroy', $profilData) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-3 mt-4">
                        <!-- Tombol Batal -->
                        <button type="button" onclick="closeDeleteModal('{{ $profilData->id }}')"
            ...
            <!-- Tombol X -->
            <button onclick="closeDeleteModal('{{ $profilData->id }}')"
